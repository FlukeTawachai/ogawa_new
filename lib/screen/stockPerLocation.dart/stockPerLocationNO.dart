...
import 'package:ogawa_nec/screen/stockPerLocation.dart/splLocationNoSort.dart';
import 'package:ogawa_nec/screen/stockPerPart.dart/sppCard.dart';

class StockPerLocation extends StatefulWidget {
  final bool autoFocusSearch;
  const StockPerLocation({Key? key, required this.autoFocusSearch})
      : super(key: key);

  @override
  State<StockPerLocation> createState() => _StockPerLocationState();
}

class _StockPerLocationState extends State<StockPerLocation> {
  ...
  void initState() {
    ...
    if (widget.autoFocusSearch == true) {
      ApiProxyParameter.splPartLocationNo = [];
    }
    if (ApiProxyParameter.splPartLocationNo.isNotEmpty) {
      uom = '';
      locationDesc = '';
      locationNo = '';
      onHand = 0;
      reserved = 0;
      available = 0;
      for (var item in ApiProxyParameter.splPartLocationNo) {
        uom = '${item.uom}';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      endDrawer: const SplLocationNoSort(),
      body: Padding(
            ...
            children: <Widget>[
              SizedBox(
                child: Row(
                  children: [
                    Text(
                      '${ApiProxyParameter.splPartLocationNo.length} record found',
              ...
              ListView.builder(
                  ...
                  shrinkWrap: true,
                  itemCount: ApiProxyParameter.splPartLocationNo.length,
                  itemBuilder: (context, index) {
                    return PathCard(ApiProxyParameter.splPartLocationNo[index],
                        index, "splPartNo");
                  })
            ],
          ),
                ...
                child: Container(
                  ...
                  child: TextField(
                    ...
                    decoration: const InputDecoration(
                      hintText: "Please input location number",
  ...
  getDataByPartNo() async {
    try {
      ...
      var result = await proxy
          .splGetDataByLocationNo(CommonReq(locationNo: search.text));
      if (result.isNotEmpty) {
        setState(() {
          ApiProxyParameter.splPartLocationNo = [];
          ApiProxyParameter.splPartLocationNo.addAll(result);
          uom = '';
          locationDesc = '';
          locationNo = '';
          onHand = 0;
